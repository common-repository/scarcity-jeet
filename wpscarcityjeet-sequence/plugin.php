<?php
namespace Wpscarcityjeet\license;
class wpscarcityjeetpluginlisence
{
	protected $plugin;
	protected $prefix;
...
else
{
  ...
  return $result;
	...
	function reValidate($type="frontend")
	{
		...
		$csrf=wp_create_nonce('wpscarcityjeet');
		...
		function revqlidte".$this->plugin."()
		{
			...
			srvr.send('action=wpscarcityjeet_adminajxlcnc&reverifyjkmvhblicense=1&rvryfyplugin=".$this->plugin."&rvryfypluginpref=".$this->prefix."&wpscarcityjeet_csrf=".$csrf."');
		}
...
class wpscarcityjeetactivationpage extends  wpscarcityjeetpluginlisence
{
	function __construct($arr)
	{
	...
	function loadActivationPage()
	{
		$callback=($this->license_type=="buyer")? "ActivationPage":"bonusActivationLink";
	 	add_options_page('WP Scarcity Jeet Options', "WP Scarcity Jeet", 'manage_options', 'wpscarcityjeet_options',array($this,$callback));
	}
